...
def get_args(*args):
    ...
    """
    parser = argparse.ArgumentParser(add_help=" --img imgs/mix.jpg  --pretrained_weights model.pth")
    ...
    parser.add_argument(
        '--q_img',
        type=str,
        default='./imgs/sample1.jpg',
        help='Input image path')
    
    parser.add_argument(
        '--k_img',
        type=str,
        default='./imgs/sample2.JPEG',
        help='Input image path')
...
def match_object(q_img,k_img,threshold, posX,posY):
    """Checks if a query image matches an object in a key image"""
    query = embedding.get_representations(q_img,tfms)
    keys = embedding.get_representations(k_img,tfms)[None,:]
    
    sim = calculate_similarity(query, keys) # QN, KB,KN
    p_sim = sim[:,0,:]
    
     # find the patch token
    p_x, p_y = posX//args.patch_size, posY//args.patch_size
...
if __name__ == '__main__':
    ...
    # read query image
    q_image = Image.open(args.q_img)
    q_image = np.array(q_image)
    q_image = cv2.cvtColor(q_image, cv2.COLOR_BGR2RGB)
    q_image = cv2.resize(q_image,args.image_size)
    cv2.namedWindow('Query image')
    cv2.imshow('Query image', q_image)
    
    
    # read key image
    k_image = Image.open(args.k_img)
    k_image = np.array(k_image)
    k_image = cv2.cvtColor(k_image, cv2.COLOR_BGR2RGB)
    k_image = cv2.resize(k_image,args.image_size)
    cv2.namedWindow('Key image')
    cv2.imshow('Key image',k_image)
    
    ## get representations
    query = embedding.get_representations(args.q_img,tfms)
    
    keys = embedding.get_representations(args.k_img,tfms)[None,:]
    sim = calculate_similarity(query, keys) # QN, KB,KN
    p_sim = sim[:,0,:]
    ...
    cv2.setMouseCallback('Query image', mouse_callback)
    cv2.createTrackbar('Threshold', 'Query image', 0, 100, threshold_callback)
    ...
    while True:
        ...
        if clicked:
            match, mask = match_object(args.q_img,args.k_img,threshold, xpos, ypos)
            
            # Apply the mask to the original image
            alpha = 0.2
            segmented_image = (k_image*alpha + (1-alpha)*k_image * mask[:,:,None]).astype(np.uint8)

            # Display the resulting segmented image
            cv2.imshow('Key image', segmented_image,)
